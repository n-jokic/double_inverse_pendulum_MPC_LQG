...
\begin{document}
	...
	\begin{itemize}
		...
		\item Horizont predikcije $N = 2500$.
		\item Matrice $\mathbf{Q}_T$, $\mathbf{Q}$ i skalar $R$ su izabrani tako da normalizuju stanja, odnosno upravljanje. 
		\item Koficijenti skaliranja: $\gamma_1 = 0.1N\Delta t$, $\gamma_2=\gamma_3=\Delta t$. 
		
	\end{itemize}, 
	pri "cemu su koficijenti skaliranja birani tako da suma aproksimira integral. Me\dj utim, bitan je samo relativan odnos ovih koeficijenata, pa mogu\'ce izabrati $\gamma_1 = 0.1N$, $\gamma_2=\gamma_3=1$. Prilikom primene \emph{multiple shooting} metode neophodno je izra"cunavanje $x(t_{i+1}; s_i; u_i)$ na osnovu po"cetne ta"cke trajektorije $s_i$, za te potrebe se primenjuje RK4 numeri"cka integracija sa korakom 1ms. \\
	
	Zbog problema sa konvergencijom u ovom radu su uvedene dve modifikacije:
	\begin{itemize}
	...
	Ostaje da se re"si pitanje prelaza bez udara. U slu"caju klasi"cnih kontrolera taj problem se elegatno re"sava uvo\dj enjem jednostavne modifikacije u originalni algoritam. U radu \cite{inicijalna} autor predla"ze formulisanje problema prelaza bez udara u formi optimizacionog problema. Za projektovani LQR "cije je upravljanje definisano kao $u = -\mathbf{K}\mathbf{x}$ treba da se minimizuje srednje kvadratno odstupanje po"cetnog stanja $\mathbf{x}$ LQR od stvarnog stanja sistema $\mathbf{x}^*$ u trenutku komutacije. Odnosno treba da se re"si slede\'ci optimizacioni problem:
	\begin{equation}
		\min_{\mathbf{x}} ||\mathbf{x} - \mathbf{x}^*||^2
	\end{equation},
	pod uslovom:
	\begin{equation}
		\mathbf{K}\mathbf{x} + u^* = 0
	\end{equation},
	gde je $u^*$ upravljanje u trenutku komutacije. U ovom zapisu problem se svodi na tra"zenje uslovnog minimuma primenom metode Lagra"zovih multiplikatora. Kao re"senje sistema se dobijaju slede'ce jedna"cine:
	\begin{align}\label{eq:komutacija}
		\lambda &= 2\frac{\mathbf{K}\mathbf{x}^*+u*}{\mathbf{K}\mathbf{K}^T} \\
		\mathbf{x} &= \mathbf{x}^*-\frac{1}{2}\lambda\mathbf{K}^T
	\end{align}.
