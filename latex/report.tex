...
\begin{document}
	...
	\begin{itemize}
		...
		\item U funkciji cene se ne koristi "clan $\gamma_2||\mathbf{x}(t_i)||^2_{\mathbf{Q}}$, pokazalo se da lo"se uti"ce na konvergenciju. Razlog za mo"ze biti to "sto u odre\dj enim delovima trajektorije treba da pove\'camo amplitudu svih stanja kako bi uspe"sno uspravili klatno.
	\end{itemize}
	...
	\begin{figure}[!h]
		...
		\label{fig:archGain}
	\end{figure} 
	Primenom zakona upravljanja oblika $\hat{\vect{u}} = - \vect{K}\hat{\vect{x}}$ transformisani sistem dovodimo u koordinatni po"cetak, u tom slu"caju va"zi va"zi $\hat{\vect{x}} = \vect{0} \rightarrow \vect{x} = \vect{x}_r$. Zakon upravljanj iz transformisanog sistema mo"zemo zapisati kao $\vect{u} = -\vect{K}(\vect{x} - \vect{x}_r) + \vect{u}_r$. Ako primenimo izvedenu relaciju iz jedna"cine (\ref{eq:ss_ff_matrix}) mo"zemo zapisati:
	\begin{equation}
	...
	\subsubsection{Stabilizacija u gornjem položaju} \label{sec:por}
